...
public class RedisMain {
    ...
    public static void main(String[] args) throws IOException {
        ...
        socket.bind(new InetSocketAddress("0.0.0.0", 16381));

        System.out.println("App is listening on 0.0.0.0:16381");
...
class RedisClone {
    ...
    String executeCommand(List<String> args) {
        switch (args.get(0).toUpperCase()) { // Ensure case-insensitive commands
            case "GET":
                return args.size() > 1 ? state.get(args.get(1)) : null;
            case "SET":
                if (args.size() > 2) {
            ...
            default:
                return "-ERR Unknown command: " + args.get(0); // Gracefully handle unknown commands
        }
    }
}
